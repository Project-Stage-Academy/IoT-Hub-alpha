...
class Device(models.Model):
    ...
    location = models.TextField(
        blank=True, null=True, help_text="Machine ID, Industrial workshop number"
    )
    status = models.CharField(
        max_length=20, choices=DeviceStatus.choices, default=DeviceStatus.ACTIVE
    )
    last_seen = models.DateTimeField(
        null=True, blank=True, help_text="Last telemetry received timestamp"
    )
    created_at = models.DateTimeField(auto_now_add=True)
    ...
    class Meta:
        ...
        indexes = [
            ...
            models.Index(
                fields=["status", "last_seen"], name="idx_device_status_last_seen"
            ),
        ]
