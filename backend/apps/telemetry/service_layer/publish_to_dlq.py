...
def publish_flush_to_dlq(producer, flush, *, reason: str, error_detail: str | None = None) -> bool:
    ...
    for p in flush:
        ...
        event_id = (
            f"{request_id}:{ingest_index}" if request_id is not None and ingest_index is not None else None
        )
