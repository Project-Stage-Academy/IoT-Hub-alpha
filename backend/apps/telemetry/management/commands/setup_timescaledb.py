...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        with connection.cursor() as cursor:
            ...
            try:
                cursor.execute(
                    "ALTER TABLE events DROP CONSTRAINT IF EXISTS "
                    "events_telemetry_id_e3b31727_fk_telemetry_id CASCADE;"
                )
            ...
            try:
                ...
                self.stdout.write(
                    self.style.WARNING(
                        f"Compression policy not added "
                        f"(columnstore not enabled): {e}"
                    )
                )
